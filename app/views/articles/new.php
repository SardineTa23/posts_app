...
if (!empty($_POST)) {
  ...
  $selected_images = array_filter($_POST, function ($var, $key) {
    return (strpos($key, 'image') !== false && $var !== "");
  }, ARRAY_FILTER_USE_BOTH);


  // 記事作成アクションの取り出し。
  $article = new Article($current_user);
  $error = $article_controller->validate($article, $selected_tags, $selected_images);
  if (!$error) {
